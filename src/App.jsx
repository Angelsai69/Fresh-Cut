...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (currentY < lastScrollY.current - 4) {
        // Scrolling UP → show header, restart timer
  ...
  // Restart hide timer on any tab change
  useEffect(() => {
    setHeaderVisible(true)
    clearTimeout(hideTimer.current)
    hideTimer.current = setTimeout(() => setHeaderVisible(false), 15000)
  ...
  return (
    <div style={{
      ...
      {/* ── TOP BAR — auto-hide after 15s, reappear on scroll-up ── */}
      <AnimatePresence>
        {headerVisible && (
          <motion.header
            key="topbar"
      ...
      <main style={{
        flex: 1,
        paddingTop: headerVisible ? 76 : 16,
        paddingBottom: 130,
        ...
        <footer style={{
          ...
          {/* Legal links — "Do Not Sell My Data" removed, "Contact Us" added */}
      ...
      {/* ── BOTTOM NAV — frosted glass 25%, no social strip ── */}
      <div style={{
        position: 'fixed',
          ...
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ fontSize: '9px', fontWeight: isActive ? 700 : 400, letterSpacing: '0.1px' }}>
                  {tab.label}
                </span>
              </motion.button>
            )
...
function ShareScreen({ t }) {
  return (
    ...
    >
      ...
      <p style={{ fontSize: tokens.font.body.size, color: t.textSub, margin: '0 0 28px' }}>
        Invite friends and earn $5 credit for every signup.
      </p>
      {/* Share card */}
      <div style={{
        background: `linear-gradient(135deg, ${tokens.colors.mintStart}, ${tokens.colors.mintEnd})`,
        ...
        <p style={{ fontSize: '16px', fontWeight: 700, color: 'white', margin: '0 0 16px', letterSpacing: '-0.2px' }}>freshcut.app/ref/alexc</p>
        <motion.button whileTap={{ scale: 0.96 }} style={{
          padding: '10px 24px', borderRadius: tokens.radius.full,
          background: 'rgba(15,43,70,0.2)', border: '1px solid rgba(255,255,255,0.3)',
      ...
      {[{ icon: '📱', label: 'Share via Messages' }, { icon: '✉️', label: 'Share via Email' }, { icon: '📋', label: 'Copy Referral Code' }].map((item) => (
        <motion.div key={item.label} whileTap={{ scale: 0.985 }} style={{
          background: t.card, borderRadius: tokens.radius.card, padding: '16px 20px', marginBottom: 10,
          display: 'flex', alignItems: 'center', gap: 16, boxShadow: t.shadow1, cursor: 'pointer',
        }}>
